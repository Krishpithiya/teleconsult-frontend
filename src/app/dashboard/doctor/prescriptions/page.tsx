...
import { FileText, Calendar, User, Pill, ChevronRight } from "lucide-react";
  ...
  useEffect(() => {
    const fetchPrescriptions = async () => {
      try {
        ...
        setTotal(res.data.total ?? res.data.prescriptions.length);
      } catch {
            ...
            {isLoading
              ...
              : prescriptions.length === 0
                ...
                : prescriptions.map((rx) => (
                  <div
                    key={rx._id}
                    className="bg-white rounded-2xl p-5 flex items-center gap-4 cursor-pointer transition-all duration-200 hover:shadow-lg"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-[#1D6FA4] flex items-center gap-1 mt-0.5">
                        <User size={12} /> {rx.patient.name}
                      </p>
                      <p className="text-xs text-[#7A90A4] mt-0.5 flex items-center gap-1">
                        <Calendar size={11} />
                        {new Date(rx.createdAt).toLocaleDateString("en-IN", {
                          day: "numeric", month: "short", year: "numeric",
                        })}
                        {rx.medicines.length > 0 && (
                          <span className="ml-2">· {rx.medicines.length} medicine{rx.medicines.length !== 1 ? "s" : ""}</span>
                        )}
                      </p>
                    </div>
